...
import { longestServices } from '../services/longest.service';
...
interface LongestService {
  getLongestVideosByPage: (page: string) => Promise<any>;
  videoList: Video[];
  totalPages: string;
}
...
let longestService = {} as LongestService;
...
const fetchVideos = async (page: string) => {
  try {
    const data = await longestService.getLongestVideosByPage(page);
    console.log(data);
    setRandomBanner(longestService.videoList as Video[]);
  } catch (e) {
...
onMounted(async () => {
  longestService = longestServices() as LongestService;
  fetchVideos(page);
});
...
watch(
  () => route.params.page,
  (newPage) => {
    fetchVideos(Array.isArray(newPage) ? newPage[0] : (newPage || '1'));
  }
);
</script>
...
<template>
  <div id="app">
    ...
    <main class="min-h-screen bg-black text-white">
      ...
      <section>
        <SliderContent :header="'Most Longest'" :items="longestService.videoList" :page="page || '1'"
          :totalPages="Number(longestService.totalPages)" :pathName="'most-longest'" />
      </section>
